export const products = [
  {
    id: 1,
    slug: "creatina-100-pure-monohydrate-1kg-foodtech-sin-sabor",
    name: "Creatine 100% pure monohydrate 1kg Foodtech sin sabor",
    description: "Creatina de alta pureza para rendimiento deportivo.",
    descriptionLong: ["Creatine 100% Pure Monohydrate de Foodtech es un suplemento alimenticio a base de monohidrato de creatina. ",
                      "Este producto está diseñado para mejorar el rendimiento físico, prevenir lesiones musculares y ayudar en la recuperación de los músculos después de entrenamientos intensos. ",
                      "La creatina monohidratada es apreciada por los deportistas debido a su rápida absorción y acción en la masa muscular. ",
                      "Este suplemento puede mejorar los resultados físicos, colaborar con el crecimiento muscular y aumentar la fuerza y energía durante el ejercicio. ",
                      "Para disfrutar de todos sus beneficios, solo debes mezclar 1 cucharadita de té o una medida (5g) con 200ml de agua, agitar y consumir."],
    image: "/images/creatina.webp",
    category: "nutricion",
    subcategory: "creatina",
      benefits: [
    "Mejora el rendimiento físico",
    "Previene las lesiones musculares",
    "Favorece el crecimiento muscular",
    "Aumenta la fuerza y energía"
    ],
    url: "https://mercadolibre.com/sec/2juCgys",
    recommended: true,
    active: true
  },
  {
    id: 2,
    slug: "omega-3-dua-labs-800-epa-400-dha-120-capsulas-para-60-dias",
    name: "Omega 3 Düa Labs® 800 Epa 400 Dha 120 Cápsulas Para 60 Días",
    description: "Omega 3 800 EPA|400 DHA 120 cápsulas Düa Labs Rinde 2 meses",
    descriptionLong: ["El Omega 3 de aceite de anchoveta del pacífico es una de las fuentes más puras de Omega 3, libre de metales pesados, máxima concentración de EPA/DHA y siendo una fuente excepcional de ácidos grasos saludables. ",
                      "Su función es apoyar tu salud cardioneurovascular, mejorar tu claridad mental, favorecer un estado de ánimo equilibrado y efectos antiinflamatorios potentes. ",
                      "Compromiso de calidad Düa Labs. ",
                      "En Düa Labs somos la primera marca chilena que realiza y publica sus análisis de laboratorio, trabajamos con estándares de calidad certificados, garantizando ingredientes de calidad, concentraciones efectivas y más porciones por envase para acompañarte por más tiempo. ",
                      ],
    image: "/images/omega.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "Mejora y fortalece la salud cardiovascular: El EPA y DHA reducen los triglicéridos en la sangre, ayudan a regular la presión arterial y mejoran la elasticidad de las arterias.",
    "Favorece la función cerebral y memoria: El DHA es un componente estructural clave del cerebro, esencial para la memoria, la concentración y la salud cognitiva.",
    "Mejora el estado de ánimo y combate el estrés regulando la producción de serotonina y dopamina",
    "Apoya la salud ocular: el DHA forma parte de la retina, ayudando a mantener una visión clara y prevenir el daño ocular asociado con la edad."
    ],
    url: "https://mercadolibre.com/sec/21bUJck",
    recommended: true,
    active: true
  },
  {
    id: 3,
    slug: "biocare-mindlinx-complex-16-billones-60-capsulas",
    name: "Biocare - Mindlinx Complex 16 Billones 60 Cápsulas",
    description: "Combinación única de probióticos y glutamina para favorecer la digestión y mantener la integridad de la pared intestinal.",
    descriptionLong: ["Combinación única de probióticos y glutamina para favorecer la digestión y mantener la integridad de la pared intestinal. ",
                      "Potencia garantizada de 16 mil millones de bacterias LAB4® y lactobacillus Rhamnosus por ingesta diaria recomendada (2 cápsulas). ",
                      "Cepas resistentes a ácidos estomacales y biliares, con alta capacidad de adherencia y de uso seguro en humanos. ",
                      "Hipoalergénico y sin derivados lácteos. Envasado al vacío para mantener su estabilidad. ",
                      "Desarrollado especialmente para personas con dificultades en el comportamiento, aprendizaje y concentración. También, participa en la recuperación de la pared intestinal y ayuda a la digestión de proteínas, siendo de gran utilidad en personas con alergias e intolerancias, y/o con enfermedades inflamatorias del aparato digestivo."],
    image: "/images/Mindlinx.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "Uso recomendado en niños, adolescentes y adultos.",
    "Libre de fibra prebiótica (FOS), siendo apto para el tratamiento de SIBO",
    "Apto para Embarazo, Lactancia, Veganos y Vegetarianos.",
    "Nuestros productos son 100% naturales"
    ],
    url: "https://mercadolibre.com/sec/1mHRqdY",
    recommended: true,
    active: true
  },
  {id: 4,
    slug: "omega-ultra-pure-150-caps-ns-sin-sabor",
    name: "Omega Ultra Pure 150 Caps, Ns Sin Sabor",
    description: "Omega 3 de alta pureza y concentración, aporta 600mg de Omega 3 por cápsula.",
    descriptionLong: ["Omega 3 de alta pureza y concentración, aporta 600mg de Omega 3 por cápsula. ",
                      "Complemento ideal para proteger y/o mejorar la salud cardiovascular y memoria. ",
                      "Cuenta con certificación IFOS, la cual garantiza pureza, seguridad y calidad del producto. ",
                      "Libre de metales pesados y contaminantes tipo PCB’s. ",
                      ],
    image: "/images/omega3UP.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "Libre de gluten",
    "Mejora la salud cardiovascular",
    ],
    url: "https://mercadolibre.com/sec/2UX8roK",
    recommended: true,
    active: true
  },
  {id: 5,
    slug: "prostar-whey-100-proteina-539-lb-ultimate-nutrition",
    name: "Prostar Whey 100% Proteína 5.39 Lb Ultimate Nutrition",
    description: "100% Whey. Disfrute de 25 gramos por porción de nuestra mezcla de péptidos, concentrado y aislado de proteína de suero de primera calidad.",
    descriptionLong: ["100% Whey garantiza una calidad incomparable en esta proteína en polvo. ",
                      "Disfrute de 25 gramos por porción de nuestra mezcla de péptidos, concentrado y aislado de proteína de suero de primera calidad. ",
                      "Diseñado para mezclarse instantáneamente, cada batido o batido ofrece una combinación perfecta de sabor y nutrición. ",
                      "Tome de 1 a 3 porciones al día. Para un crecimiento muscular máximo, tómalo antes y después de tu entrenamiento. ",
                      "Mezcle 1 cucharada (30 gramos) de Ultimate Nutrition Prostar® 100% Whey Protein en 8 oz de agua o leche en una licuadora."],
    image: "/images/prostar.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Mejora el crecimiento muscular",
    "Favorece la recuperación post entrenamiento",
    "Aumenta la fuerza y energía"
    ],
    url: "https://mercadolibre.com/sec/2Kj6zoW",
    recommended: true,
    active: true
  },
  {id: 6,
    slug: "dymatize-proteina-de-iso-100-sabor-vainilla",
    name: "Dymatize Proteína de ISO-100 sabor vainilla",
    description: "ISO 100 tiene todo lo que nuestro cuerpo necesita para construir y mantener la masa muscular de manera efectiva.",
    descriptionLong: ["ISO 100 tiene todo lo que nuestro cuerpo necesita para construir y mantener la masa muscular de manera efectiva. ",
                      "La proteína de suero se ha hidrolizado para un uso más eficiente en el cuerpo, debido a su estructura más pequeña ofrece la reposición de aminoácidos de manera rápida. ",
                      "Cada porción de ISO 100 proporciona 25 gramos de proteína hidrolizada de suero 100% aislado, con menos de un gramo de azúcar y grasa. ",
                      "Además entrega 5,5 gramos de aminoácidos de cadena ramificada (BCAA), que son utilizados por el cuerpo durante el entrenamiento intenso. ",
                      "Estos nutrientes son importantes para favorecer el crecimiento muscular y la recuperación post entrenamiento."],
    image: "/images/dymatize.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Aminoácidos esenciales para mejorar el rendimiento deportivo.",
    "Favorece la recuperación muscular post entrenamiento.",
    "Libre de gluten para una mayor accesibilidad.",
    "Apto para personas en período de lactancia y embarazo"
    ],
    url: "https://mercadolibre.com/sec/2RLhwNv",
    recommended: true,
    active: true
  },
  {id: 7,
    slug: "wild-protein-whey-187-kg-chocolate",
    name: "Wild Protein Whey 1,87 Kg - Chocolate",
    description: "Wild Protein Whey, la primera proteína certificada por el INTA en Chile.",
    descriptionLong: `Wild Protein Whey, la primera proteína certificada por el INTA en Chile. Para todos los que buscan aumentar su consumo de proteínas con una opción rica, práctica y de calidad. Esta certificación garantiza un perfil completo de aminoácidos esenciales y una alta digestibilidad, lo que permite que el cuerpo utilice al máximo sus proteínas.


¿QUÉ HAY EN CADA SCOOP?

25 g de proteína concentrada de suero de leche (whey)
5 g de BCAA
2,6 g de leucina
Sin azúcar añadida
Sin sellos
Fácil disolución y alta versatilidad
Análisis de digestibilidad proteica del 100%`,
    image: "/images/wildprotein.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Crecimiento y reparación muscular.",
    "Previene la recuperación muscular.",
    "Favorece el rendimiento.",
    "Aumenta la saciedad. "
    ],
    url: "https://mercadolibre.com/sec/2ME5NtD",
    recommended: true,
    active: true
  },
  {id: 8,
    slug: "dymatize-whey-iso-100-23kg-en-polvo-sabor-fudge-brownie",
    name: "Dymatize Whey ISO-100 2.3kg en Polvo Sabor Fudge Brownie",
    description: "ISO-100® está diseñada para deportistas exigentes, proporcionando lo necesario para construir y mantener masa muscular de manera efectiva.",
    descriptionLong: `ISO-100® está diseñada para deportistas exigentes, proporcionando lo necesario para construir y mantener masa muscular de manera efectiva. La proteína de suero se ha hidrolizado para un uso más eficiente, ofreciendo reposición rápida de aminoácidos debido a su estructura pequeña. Disponible en envases de 1.6 lbs (744g), 3 lbs (1.395g) y 5 lbs (2.270g) en sabores Chocolate, galleta y crema. Se recomienda consumir antes de un entrenamiento intenso mezclando 3 cucharadas con 350 ml de agua fría y consumir 2 veces al día.

COMPONENTES:
Cada porción de ISO-100® aporta 25 gramos de proteína hidrolizada de suero 100% aislado, con menos de un gramo de azúcar y grasa. Incluye 5,5 gramos de aminoácidos de cadena ramificada (BCAA), con 2,7 gramos de BCAA L-leucina.

PRECAUCIONES
Contiene leche y soya.`,
    image: "/images/dymatizebrownie.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Aminoácidos esenciales para mejorar el rendimiento deportivo.",
    "Favorece la recuperación muscular post entrenamiento.",
    "Libre de gluten para una mayor accesibilidad.",
    "Apto para personas en período de lactancia y embarazo"
    ],
    url: "https://mercadolibre.com/sec/1RfkJZB",
    recommended: true,
    active: true
  },
  {id: 9,
    slug: "magmax-4-magnesios-citrato-l-treonato-taurato-bisglicinato-sabor-sin-sabor",
    name: "Magmax 4 Magnesios Citrato L Treonato Taurato Bisglicinato Sabor Sin Sabor",
    description: "Promueve la salud cardiovascular, ósea y muscular, además de ayudar a mantener un sistema nervioso saludable.",
    descriptionLong:  `¿Para qué Enfermedades o Condiciones se prescribe este Medicamento?
Su formulación está especialmente hecha para un apoyo integral. Promueve la salud cardiovascular, ósea y muscular, además de ayudar a mantener un sistema nervioso saludable, favoreciendo tanto en la actividad como en la relajación muscular.

¿Cómo debo usar este Producto? (Posología)
Ingerir con agua, se recomienda ingerir con alimentos. Consumir 1 porción (2 comprimidos) al día, no consumir más de una porción al día.

Presentación (Forma Farmacéutica y Dosis)
Frasco de aluminio aprobado por la FDA, que contiene 60 comprimidos.

300mg de Magnesio elemental por comprimido.
L-Treonato de Magnesio 60mg.
Citrato de magnesio 150mg.
Bisglicinato de magnesio 50mg.
Taurato de magnesio 40mg.
`,
    image: "/images/magma.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "L-Treonato de Magnesio 60mg para salud cognitiva.",
    "Citrato de magnesio 150mg para soporte muscular.",
    "Bisglicinato de magnesio 50mg para mejor absorción.",
    "Taurato de magnesio 40mg para salud cardiovascular."
    ],
    url: "https://mercadolibre.com/sec/1RNeDfY",
    recommended: true,
    active: true
  },
  {id: 10,
    slug: "colageno-hidrolizado-artrosome-vitamina-c-y-d-30-sobres-sin-sabor",
    name: "Colágeno Hidrolizado Artrosome + Vitamina C Y D, 30 Sobres Sin Sabor",
    description: "Artrosome C+D Colágeno Hidrolizado 30 Sobres, Suplemento alimentario en polvo a base de colágeno hidrolizado 10.000 mg.",
    descriptionLong: `Artrosome C+D Colágeno Hidrolizado 30 Sobres. Suplemento alimentario en polvo a base de colágeno hidrolizado 10.000 mg. Excelente fuente de Vitamina C y D. Sin azúcares añadidos.
 Los suplementos que contienen colágeno hidrolizado contribuyen a mantener y recuperar la salud articular; por eso se recomiendan a personas que, por su edad o actividad física, tienen un mayor riesgo de presentar daño en las articulaciones.`,
    image: "/images/artrosome.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "Estimula la producción de colágeno en el organismo",
    "Ayuda a mejorar el aspecto de la piel",
    "Fortalece las articulaciones, las uñas, los huesos y el cabello"
    ],
    url: "https://mercadolibre.com/sec/1WxbvbL",
    recommended: true,
    active: true
  },
  {id: 11,
    slug: "biocare-vitamina-d3-k2-15ml-sistema-inmune",
    name: "Biocare - Vitamina D3 + K2 15ml (sistema Inmune)",
    description: `Nutrisorb D3K2 líquido proporciona vitamina D3 y vitamina K2 en una base simple de triglicéridos de cadena media.
Ingredientes:
- Triglicéridos de cadena media
- Vitamina K2 (Menaquinona-7)
- Vitamina D3 (Colecalciferol)
- Antioxidante (D-alfa tocoferol)

Ingesta Diaria:

- 6 gotas diarias proporcionan Vitamina D3 = 1000UI – 25ug , Vitamina K2 75ug. Suministro para 83 días al dar 6 gotas diarias.

Vitamina D3 = 1000UI – 25ug , Vitamina K2 75ug
`,
    image: "/images/nutrisorb.webp",
    category: "nutricion",
    subcategory: "suplementos",
    benefits: [
    "Formulación vegana líquida de alta potencia y óptima absorción",
    "Apoya el sistema inmune, la absorción de calcio, la formación de huesos y la función cardiovascular.",
    "Contribuye a la absorción y utilización de calcio y fósforo.",
    "Regula los niveles de calcio en la sangre.",
    "Mantención de la función y estructura muscular"
    ],
    url: "https://mercadolibre.com/sec/2rEJUp5",
    recommended: true,
    active: true
  },
  {id: 12,
    slug: "creatine-basics-nutrition-creatina-100-creapure-sabor-sin-sabor",
    name: "Creatine+ Basics Nutrition - Creatina 100% Creapure® Sabor Sin sabor",
    description: "La creatina monohidratada más pura del mundo, fabricada en Alemania y respaldada por ciencia. Basics Nutrition trae a Chile la creatina Creapure® con la mayor pureza y biodisponibilidad.",
    descriptionLong: `CREATINE+ — 100% Creapure® La creatina monohidratada más pura del mundo, fabricada en Alemania y respaldada por ciencia. Basics Nutrition trae a Chile la creatina Creapure® con la mayor pureza y biodisponibilidad.
    Ideal para:
Entrenamiento de alta intensidad y momentos de alta concentración. La creatina es esencial para la producción de energía a nivel celular, tanto muscular como cerebral.

Dosis recomendada: 1 scoop colmado (5 g) en 300 ml de agua. Con un consumo diario, el producto rinde hasta 2 meses.

Máxima pureza: Creapure® es fabricada por Alzchem Trostberg GmbH en una planta dedicada exclusivamente a la producción de creatina de calidad superior.

Compra con confianza: CREATINE+ asegura rendimiento y salud, con el respaldo de la ciencia.
`,
    image: "/images/creatina1.webp",
    category: "nutricion",
    subcategory: "creatina",
    benefits: [
    "Mejora el rendimiento deportivo.",
    "Aumenta la energía y acelera la recuperación.",
    "Favorece la ganancia muscular.",
    "Mejora la concentración y capacidad cognitiva."
    ],
    url: "https://mercadolibre.com/sec/1zJw6HG",
    recommended: true,
    active: true
  },
  {id: 13,
    slug: "suplemento-en-polvo-winkler-nutrition-whey-pro-win-proteinas-sabor-capuccino-en-pote-de-1kg",
    name: "Suplemento en polvo Winkler Nutrition Whey Pro Win proteínas sabor capuccino en pote de 1kg",
    description: "WHEY PRO WIN es 100% suero de leche, incluyendo concentrados, aislados e hidrolizados. Contribuye a la recuperación muscular óptima post-entrenamiento.",
    descriptionLong: `WHEY PRO WIN es 100% suero de leche, incluyendo concentrados, aislados e hidrolizados. Contribuye a la recuperación muscular óptima post-entrenamiento. Enriquecido con L-Glutamina, BCAA, L-Leucina y enzima lactasa.

Sabores disponibles: Chocolate suizo, Dulce de leche, Cappucino, Menta chips, Vainilla, Pie de limón y Frutos del bosque.

Cantidad de porciones: 30

Instrucciones de consumo: Disolver 1 scoop (33g) en 300 ml de agua o leche.
Sugerencias de uso: Ingerir posterior a la actividad física.
`,
    image: "/images/w1whey.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Aminoácidos esenciales para mejorar el rendimiento deportivo.",
    "Favorece la recuperación muscular post entrenamiento.",
    "Favorece el crecimiento muscular",
    "Aumenta la fuerza y energía"
    ],
    url: "https://mercadolibre.com/sec/2nZ2SPN",
    recommended: true,
    active: true
  },
  {id: 14,
    slug: "proteína-de-suero-foodtech-100-women-frutos-del-bosque-con-biotina-y-colageno-907g-en-polvo",
    name: "Proteína de Suero Foodtech 100% Women Frutos del Bosque con Biotina y Colágeno 907g en Polvo",
    description: "100% Women Whey Protein + Biotin & Collagen la proteína favorita de la mujer ahora más completa, gracias a su nueva y mejorada fórmula.",
    descriptionLong: `100% Women Whey Protein + Biotin & Collagen la proteína favorita de la mujer ahora más completa, gracias a su nueva y mejorada fórmula que te entrega una proteína concentrada de suero de calidad, enriquecida con biotina y colágeno. En cada batido nutrirás tu cuerpo con toda la tecnología que Foodtech te entrega.

Instrucciones de uso:

Disolver 1 medida dosificadora en 250 ml de agua fría o líquido a gusto, agitar y consumir inmediatamente.

Precauciones

“ALIMENTO PARA DEPORTISTAS ALTO EN PROTEÍNAS CON ADICIÓN DE BIOTINA”.

NO RECOMENDABLE PARA MENORES DE 15 AÑOS, EN EMBARAZO NI LACTANCIA.
`,
    image: "/images/womenwhey.webp",
    category: "nutricion",
    subcategory: "proteinas",
    benefits: [
    "Contiene proteínas concentradas de suero.",
    "Enriquecida con biotina y colágeno.",
    "Aporta 22g de proteínas por porción.",
    "No contiene azúcares añadidos."
    ],
    url: "https://mercadolibre.com/sec/2eY6YXo",
    recommended: true,
    active: true
  },
  {id: 15,
    slug: "set-mancuernas-y-barra-unibles-15kg-fitness-pesas-ajustables-overfit",
    name: "Set Mancuernas Y Barra Unibles 15kg Fitness Pesas Ajustables Overfit",
    description: " El set de mancuernas y barra Overfit de 15 kg es la solución perfecta para ti, más fuerza y menos espacio.",
    descriptionLong: `¿Quieres entrenar en casa sin llenar tu living de equipos voluminosos? El set de mancuernas y barra Overfit de 15 kg es la solución perfecta para ti, más fuerza y menos espacio.

Este kit te permite ejercitar brazos, hombros, espalda y piernas combinando diferentes pesos y posiciones, logrando rutinas completas sin salir de tu hogar.

Su práctico diseño modular te permite usarlo como mancuernas para un trabajo localizado o unirlas con el conector para formar una barra, ideal para entrenamientos compuestos.

Además, ocupa poco espacio, por lo que podrás guardarlo fácilmente cuando termines.

Características:

Versatilidad 2 en 1, úsalo como mancuernas o barra larga con el conector incluido.

Discos recubiertos para mayor durabilidad y cuidado de tus superficies.

Empuñaduras con textura antideslizante para un agarre seguro y cómodo.

Incluye tuercas de tornillo para fijar los discos firmemente.

Ahorra espacio en tu hogar, reemplazando varios equipos por un solo set. `,
    image: "/images/pesas1.webp",
    category: "equipamento",
    subcategory: "pesas",
    benefits: [
    "Aumentar fuerza y trabajar distintos grupos musculares.",
    "Ideal para tonificar.",
    "Ahorra espacio en tu hogar, reemplazando varios equipos por un solo set.",
    "Ahora es posible entrenar de forma económica, práctica y sin excusas."
    ],
    url: "https://mercadolibre.com/sec/2noBwDC",
    recommended: true,
    active: true
  },
  {id:16,
    slug: "trotadora-tm40-lite-ten-series-xmas-edition",
    name: "Trotadora Tm40 Lite Ten Series Xmas Edition",
    description: "Diseñada para quienes buscan comodidad y eficiencia, la TM40 Lite Ten Series combina tecnología inteligente con un diseño ultradelgado",
    descriptionLong: `Trotadora TM40 Lite Ten Series – Xmas Edition.
    La Trotadora TM40 Lite Ten Series – Xmas Edition reúne todo lo que distingue a la TM40 Lite con un toque especial para esta temporada navideña. Mantiene su diseño ultradelgado, su tecnología compacta y su funcionamiento silencioso, pero ahora llega en una edición limitada perfecta para sorprender o regalar salud, fitness y bienestar durante las fiestas.

Diseñada para integrarse en cualquier hogar, la TM40 Lite ofrece un formato ligero y eficiente: mide solo 12 cm de alto, 49 cm de ancho y 120 cm de largo, con un peso de apenas 16 kg, lo que permite guardarla fácilmente bajo muebles o moverla sin esfuerzo. Su motor de 1.5 HP de potencia máxima (0.65 HP continua) permite entrenamientos de 1.0 a 8 km/h, ideales para caminatas, pasos rápidos o trotes suaves. La superficie de trote de 96 × 38,5 cm brinda estabilidad y seguridad, soportando usuarios de hasta 100 kg. `,
image: "/images/trotadora.webp",
    category: "equipamento",
    subcategory: "maquinas",
    benefits: [
    "Cuenta también con sistema Antishock.",
    "Incorpora inclinación manual de tres niveles.",
    "Soportando usuarios de hasta 100 kg.",
    ],
    url: "https://mercadolibre.com/sec/1pbUn8Y",
    recommended: true,
    active: true
  },
  {id: 17,
    slug: "reloj-smartwatch-xiaomi-band-10-172-amoled-bluetooth-resistencia-5atm-midnight-black",
    name: "Reloj Smartwatch Xiaomi Band 10 1.72 Amoled Bluetooth Resistencia 5atm Midnight Black",
    description: "XIAOMI Band 10 pantalla AMOLED de 1.72 ofrece colores vibrantes y una excelente visibilidad en exteriores. Su resolución de 212 x 520 píxeles y tasa de refresco de 60Hz",
    descriptionLong: `Detalles Técnicos
-Pantalla 1.72" AMOLED
-Resolución: 212 x 520px
-Tasa de refresco 60Hz
-Bluetooth 5.4
-Resistente al agua hasta 50 m
-Batería de Li-Po, 233 mAh
-Carga magnética
-Correa: TPU
-Aplicación Mi Fitness
-Peso 15,95g

Sistema Operativo
-Android 8.0 o iOS 14.0 y superior

Sensores
-Acelerómetro
-Giroscopio
-Brújula electrónica
-Oxímetro óptico de frecuencia cardíaca y pulso
-Sensor de luz ambiental

Diseño compacto, pantalla que impacta
La nueva XIAOMI Band 10 te sorprende con una pantalla AMOLED de 1.72" que ofrece colores vibrantes y una excelente visibilidad en exteriores. Su resolución de 212 x 520 píxeles y tasa de refresco de 60Hz garantizan una experiencia visual fluida y nítida.`,

    image: "/images/relojxiaomi.webp",
    category: "accesorios",
    subcategory: "accesorios",
    benefits: [
    "Monitorea tu bienestar en tiempo real.",
    "Medidor de frecuencia cardíaca",
    "Resistente al agua.",
    ],
    url: "https://mercadolibre.com/sec/2QVGW9e",
    recommended: true,
    active: true
  },
  {id: 18,
    slug: "zapatillas-adidas-casual-cloudfoam-comfy-hombre-ih2973",
    name: "Zapatillas adidas Casual Cloudfoam Comfy Hombre Ih2973",
    description: "Comodidad que nunca falla.",
    descriptionLong: `Comodidad que nunca falla. Estas zapatillas adidas te permiten moverte con total libertad cuando sales a la calle sin comprometer tu estilo. La parte superior ligera trae un forro interno que envuelve el pie en comodidad. Una plantilla Cloudfoam Comfort proporciona amortiguación suave mientras que la suela duradera se adhiere al suelo para brindarte confianza sin importar a donde te lleve el día.

Detalles

. Horma clásica

. Cierre de cordones

. Exterior textil

. Forro textil

. Forro textil

. Plantilla Cloudfoam Comfort

. Suela sintética`,
    image: "/images/adidascloud.webp",
    category: "accesorios",
    subcategory: "calzado",
    url: "https://mercadolibre.com/sec/1Sgxaeb",
    recommended: true,
    active: true
  },
  {id: 19,
    slug: "carpa-hexagonal-automatica-camping-6-personas-220x264x150-beige",
    name: "Carpa Hexagonal Automática Camping 6 Personas 220x264x150 Beige",
    description: "Carpa automática hexagonal para 6 personas. Armado fácil en 1 minuto queda lista para usar.",
    descriptionLong: `Carpa automática hexagonal para 6 personas. Armado fácil en 1 minuto queda lista para usar.

Carpa automática fácil y rápida de armar
Impermeable: 2500 mm
Incluye bolsa ligera con manija para transporte, con todos sus accesorios
Ventanas de ventilación en los 4 lados y puerta delantera
Capacidad para 6 personas, con costuras reforzadas, techo resistente y suelo que aísla el frío y la humedad

Estación: 3 estaciones
Capacidad: 6 personas
Color: Beige
Dimensiones montado: 240x280x160 cm`,
    image: "/images/carpa.webp",
    category: "equipamento",
    subcategory: "outdoor",
    url: "https://mercadolibre.com/sec/27k7ZC8",
    recommended: true,
    active: true
  },
  {id: 19,
    slug: "bicicleta-aro-16-infantil-con-ruedas-de-aprendizaje-rosa-chicle-16",
    name: `Bicicleta Aro 16 Infantil Con Ruedas De Aprendizaje Rosa Chicle 16" `,
    description: "La bicicleta aro 16 con ruedas de aprendizaje es ideal para niñas que están aprendiendo a andar en bicicleta.",
    descriptionLong: `La bicicleta aro 16 con ruedas de aprendizaje es ideal para niñas que están aprendiendo a andar en bicicleta. Fabricada con materiales de hierro de alta calidad, esta bicicleta de paseo de la marca China es resistente y duradera. Con frenos delanteros tipo caliper, ofrece seguridad y facilidad de uso para las más pequeñas. Además, cuenta con ruedas de entrenamiento para una mayor estabilidad y un canasto para llevar sus objetos favoritos durante el paseo. Con un peso máximo soportado de 40 kg, este modelo de aro 16 es fácil de transportar. ¡Una excelente opción para que las niñas disfruten de sus primeras aventuras en bicicleta!

Garantía de fábrica: 1 meses`,
    image: "/images/biciniña.webp",
    category: "equipamento",
    subcategory: "outdoor",
    url: "https://mercadolibre.com/sec/28tSqjc",
    recommended: true,
    active: true
  },
  {id: 20,
    slug: "bicicleta-bianchi-freestyle-sport-bmx-tamaño-del-cuadro-talla-unica-rodado-20-color-gris-plata",
    name: "Bicicleta Bianchi Freestyle Sport bmx Tamaño del Cuadro Talla Unica rodado 20 color Gris Plata",
    description: "Es la compañera ideal para quienes buscan estilo urbano, resistencia y maniobrabilidad en cada recorrido",
    descriptionLong: `La bicicleta Bianchi Freestyle Sport BMX gris plata talla única es la compañera ideal para quienes buscan estilo urbano, resistencia y maniobrabilidad en cada recorrido. 

    Su estructura robusta está diseñada para soportar exigencias propias del freestyle y del uso diario. Está equipada con llantas de aro 20 que brindan una excelente respuesta en terreno urbano, permitiendo efectuar maniobras con soltura. Los pedales Wellgo ofrecen agarre seguro, mientras los puños de goma negra proporcionan confort al sujetar el manubrio. El asiento Soft en color negro asegura comodidad en desplazamientos prolongados. Viene con accesorios útiles como reflectantes y campanilla para mayor visibilidad y seguridad. Su tono gris plata le otorga un acabado moderno, combinable con diversos estilos. Ideal tanto para jóvenes entusiastas del BMX como para quienes disfrutan rodar por la ciudad con un toque deportivo.`,
    image: "/images/biciniño.webp",
    category: "equipamento",
    subcategory: "outdoor",
    url: "https://mercadolibre.com/sec/1emsnmg",
    recommended: true,
    active: true
  },
  {id: 21,
    slug: "bicicleta-oxford-infantil-spine-bmx-aro-20-serie-2025-color-verde-tamaño-del-cuadro-tamaño-único",
    name: "Bicicleta Oxford Infantil Spine Bmx Aro 20 Serie 2025 Color Verde Tamaño Del Cuadro Tamaño Único",
    description: "Destaca por su diseño atractivo y materiales de excelente calidad.",
    descriptionLong: `Cantidad de velocidades: 1.
Género: Niños.
Tamaño del cuadro: Tamaño Único.
Aro: 20.
Material del cuadro: marco de acero.
Dispone de manubrio gyro 360, acero high-ten, tipo bmx 660 mm ancho.
Con frenos: U-Brake Aluminio.
Peso: 13.55kg.`,
    image: "/images/bicioxford.webp",
    category: "equipamento",
    subcategory: "outdoor",
    url: "https://mercadolibre.com/sec/2ord7za",
    recommended: true,
    active: true
  },
  {id: 22,
    slug: "zapatillas-galaxy-6-gw3848-adidas",
    name: "Zapatillas Galaxy 6 Gw3848 adidas",
    description: "Estas zapatillas de running adidas mantiene tus pies cómodos sin importar qué tan tarde salgas a correr.",
    descriptionLong: `Logra tus objetivos con este calzado. Estas zapatillas de running adidas mantiene tus pies cómodos sin importar qué tan tarde salgas a correr. Perfectas para el uso diario, tienen un exterior de malla transpirable y ligero que brinda frescura a tus pies y una mediasuela Cloudfoam para pasos más cómodos. La suela de caucho está diseñada para mantenerse firme en todo tipo de superficies, desde pasto mojado hasta arcilla. Cambia de planes sin tener que cambiar de zapatillas.

Este producto, hecho con una serie de materiales reciclados, cuenta con un exterior que incorpora al menos un 50 % de contenido reciclado. Este producto representa solo una de nuestras soluciones para acabar con los residuos plásticos.`,
    image: "/images/adidasgalaxy.webp",
    category: "accesorios",
    subcategory: "calzado",
    url: "https://mercadolibre.com/sec/2zTFNZ3",
    recommended: true,
    active: true
  },
  {id: 23,
    slug: "zapatillas-duramo-sl-20-gw8342-adidas",
    name: "Zapatillas Duramo Sl 2.0 Gw8342 adidas",
    description: "Mantén tus pies cómodos y tu estilo impecable en todo momento con estas zapatillas adidas.",
    descriptionLong: `Mantén tus pies cómodos y tu estilo impecable en todo momento con estas zapatillas adidas. Úsalas con tus shorts de running favoritos para lucir un look deportivo y casual. El exterior de malla ayuda a mantener tus pies frescos, mientras que la amortiguación ultraliviana te ofrece soporte en cada uno de tus pasos.

Hechas con una serie de materiales reciclados, su exterior incorpora al menos un 50 % de contenido reciclado. Este producto representa solo una de nuestras soluciones para acabar con los residuos plásticos.`,
    image: "/images/adidasduramo.webp",
    category: "accesorios",
    subcategory: "calzado",
    url: "https://mercadolibre.com/sec/1S4b3Q8",
    recommended: true,
    active: true
  },
  {id: 24,
    slug: "pared-electronica-de-boxeo-ejercicio-digital-guantes-infantil",
    name: "Pared Electronica De Boxeo Ejercicio Digital+guantes Infantil",
    description: "Novedosa y excelente maquina de boxeo musical bluetooht, recargable, incluye guantes",
    descriptionLong: `recargable
incluye guantes
trae cinta doble pegante el equipo para fijarlo , tambien incluye 3 chupones para superficies planas

3 botones centrales
boton encendido y apagado y coneccion bluetooht
boton de volumen 3 niveles de volumen
boton de circuito de entrenamiento
con lineas rojas de movimiento para dar el golpe
luces azul cuando das el golpe en cada lugar
cargador usb
dura aprox 2 horas de uso
se carga en 2.5 horas
incluye cable usb`,
    image: "/images/boxpared.webp",
    category: "equipamento",
    subcategory: "maquinas",
    url: "https://mercadolibre.com/sec/1EyNWxF",
    recommended: true,
    active: true
  },
  {id: 25,
    slug: "zapatillas-bsketball-unisex-lckdown7-negro",
    name: "Zapatillas Bsketball Unisex Lckdown7 Negro",
    description: " Inspiramos a todo apasionado del deporte con soluciones de alto rendimiento.",
    descriptionLong: `Under Armour te hace mejor. En eso nos enfocamos. Inspiramos a todo apasionado del deporte con soluciones de alto rendimiento que no sabías que necesitabas, pero que no te puedes imaginar viviendo sin ellas. Todo lo que hacemos gira en torno al rendimiento humano, entregando lo necesario para empujar más allá de cualquier límite. Todos los productos que creamos se hacen para hacer mejor a todos los deportistas.
    * Calzado publicado en talla chilena, por favor revisar la guía de tallas publicada en las fotos del producto para encontrar tu talla ideal en base a los CM de tu pie
* Zapatillas Bsketball unsex Lckdown7 Negro Under Armour
* Marca: Under Armour
* Modelo: UA LOCKDOWN 7
* Parte superior de malla transpirable con superposiciones de cuero para mayor soporte y durabilidad.
* La entresuela de EVA ofrece una pisada ligera y reactiva
* Suela exterior de goma sólida y duradera con patrón de tracción en espiga para máximo movimiento y control en la cancha.
* Estilo #: 3028512
* Tipo de encaje: Cordones
* Importado.
* REGULAR
* Son ligeras, cómodas y lo suficientemente resistentes como para durar toda la temporada, lo que las convierte en las zapatillas para la cancha de basketball año tras año.`,
    image: "/images/ld7.webp",
    category: "accesorios",
    subcategory: "calzado",
    url: "https://mercadolibre.com/sec/21wX65e",
    recommended: true,
    active: true
  },
  {id: 26,
    slug: "mancuerna-hexagonal-10kg-par-set-x2-pesas-pvc-20kg-overfit-color-gris",
    name: "Mancuerna Hexagonal 10kg Par Set X2 Pesas Pvc 20kg Overfit Color Gris",
    description: "La mancuerna tiene su peso claramente marcado, facilitando la selección rápida y precisa durante tus sesiones de ejercicio.",
    descriptionLong: `La mancuerna tiene su peso claramente marcado, facilitando la selección rápida y precisa durante tus sesiones de ejercicio.

Su forma hexagonal evita que ruede, proporcionando mayor estabilidad y seguridad al usarla.

Fabricada con cemento y PVC de alta densidad, garantiza durabilidad y resistencia al desgaste.

El mango de PVC ofrece una excelente resistencia a la corrosión y un aspecto elegante.

Con esta mancuerna tendrás un manejo firme y cómodo, incluso durante los entrenamientos más intensos.

Su diseño facilita el almacenamiento eficiente, permitiendo un orden óptimo en tu espacio de entrenamiento.
`,
    image: "/images/pesasoverfit.webp",
    category: "equipamento",
    subcategory: "pesas",
    benefits: [
    "Aumentar tu fuerza.",
    "Tonifica tus músculos o mejora tu resistencia",
    "Optimiza tu espacio.",
    "Combina ejercicios y entrena tu cuerpo completo con la máxima seguridad y comodidad.",
    ],
    url: "https://mercadolibre.com/sec/32Cu685",
    recommended: true,
    active: true
  },
  {id: 27,
    slug: "termo-stanley-adventure-to-go-1-litro-con-tapon-cebador-frost",
    name: "Termo Stanley Adventure To-go 1 Litro Con Tapón Cebador Frost",
    description: "Diseñada para los días de trabajo, vacaciones y todos los días, la botella Adventure To-Go es moderna en su diseño y duradera en su rendimiento.",
    descriptionLong: `Termo Stanley Adventure TO-GO 1 Litro con Tapón Cebador.
Diseñada para los días de trabajo, vacaciones y todos los días, la botella Adventure To-Go es moderna en su diseño y duradera en su rendimiento. El estilo aerodinámico permite empacar y transportar cómodamente, perfecto para cualquier aventura en interiores o exteriores. Mantenga sus bebidas favoritas calientes o frías durante horas con el aislamiento al vacío de doble pared de Stanley. Esta elegante botella incluye un tapón de vertido para un mayor control y evitar salpicaduras. La tapa aislada también funciona como taza para beber fácilmente donde quiera que vaya.

Características:

> Con Sistema de Tapón Cebador
>Fabricado en acero inoxidable 18/8, naturalmente libre de BPA.
>Doble pared aislante para el mantenimiento de temperatura
>Tapa de aislamiento térmico, también puede ser utilizada como taza.
>Mantiene las bebidas calientes por 25 hs y frías por 27 hs. Helado por 5 días.
>A prueba de fugas.
>Ideal para transportar.
>Apto lavavajillas.
>Peso producto: 640 g.
>Medidas: 30 cm de alto, 9 de ancho.`,
    image: "/images/stanley2.webp",
    category: "accesorios",
    subcategory: "accesorios",
    url: "https://mercadolibre.com/sec/2mgZPhb",
    recommended: true,
    active: true
  },
  {id: 28,
    slug: "banco-multifuncional-magnux-pesas-reclinable-predicador-multiposicion-graduable",
    name: "Banco Multifuncional Magnux Pesas Reclinable Predicador Multiposicion Graduable",
    description: "¡Descubre el Banco Magnux Multiposicional! Tu compañero ideal para esculpir un abdomen de acero, fortalecer la zona lumbar y desafiar tus límites en cada entrenamiento.",
    descriptionLong: `¡Descubre el Banco Magnux Multiposicional! Tu compañero ideal para esculpir un abdomen de acero, fortalecer la zona lumbar y desafiar tus límites en cada entrenamiento.

Con este banco versátil, podrás realizar una variedad de ejercicios, desde trabajo abdominal y lumbar hasta ejercicios de pecho, hombros y dorsales. ¡Tu creatividad será tu única limitación!

Diseñado para brindarte comodidad y durabilidad, este banco cuenta con cojinería indeformable que te asegura una experiencia de entrenamiento óptima. Su diseño plegable te permite ahorrar espacio cuando no esté en uso, perfecto para hogares o gimnasios con espacio limitado.

¡Y eso no es todo! Con su capacidad multiangular, puedes ajustar fácilmente la posición del banco para realizar ejercicios inclinados, declinados, rectos o planos, adaptándose a tus necesidades y metas de entrenamiento.

¡Es hora de llevar tu entrenamiento al siguiente nivel con el Banco Magnux Multiposicional! Atrévete a desafiar tus límites y alcanzar tus objetivos de fitness de manera eficiente y emocionante.`,
    image: "/images/bancomagnux.webp",
    category: "equipamento",
    subcategory: "maquinas",
    url: "https://mercadolibre.com/sec/1ZijC2x",
    recommended: true,
    active: true
  },
  {id: 29,
    slug: "set-mancuernas-ajustables-20kg-importacion-con-barra-y-discos-negro",
    name: "Set Mancuernas Ajustables 20kg Importación con Barra y Discos Negro",
    description: "Este set de mancuernas de 20kg con barra armable ajustable y discos en color negro, ofrece un agarre texturizado cómodo que previene resbalones durante el ejercicio, asegurando tu seguridad.",
    descriptionLong: `Este set de mancuernas de 20kg con barra armable ajustable y discos en color negro, ofrece un agarre texturizado cómodo que previene resbalones durante el ejercicio, asegurando tu seguridad.

Es perfecto para entrenamientos corporales, ayudando a tonificar y esculpir tus brazos, hombros, espalda y a fortalecer tus músculos. Además, el entrenamiento con mancuernas ajustables contribuye a aumentar tu tasa de metabolismo, lo que puede resultar en una mayor quema de grasas.

Mancuernas Ajustables:

PESO TOTAL 20 KG
4 discos: 1,25 Kg. (18 cm de diámetro).
4 discos: 1,5 Kg. (19 cm de diámetro).
4 discos: 2 Kg (20 cm de diámetro).
2 barras: (37 cm de longitud).
Extensor de 40 cm acolchado. (4 cm de diámetro).

Puedes ajustar los discos según el peso que necesites. Los discos están hechos de cemento revestido y el mango es de metal con Pvc.`,
    image: "/images/pesas2.webp",
    category: "equipamento",
    subcategory: "pesas",
    url: "https://mercadolibre.com/sec/1D2nnAR",
    recommended: true,
    active: true
  },
  {id: 30,
    slug: "anillas-olimpicas-fitness-calistenia-correa-+-hebillas-eshopangie",
    name: "Anillas Olímpicas Fitness Calistenia Correa + Hebillas Eshopangie",
    description: "Los anillos están hechos de madera de alta calidad, que es cómoda y estable. Con excelente agarre, alta capacidad de carga y durabilidad. ",
    descriptionLong: `Los anillos están hechos de madera de alta calidad, que es cómoda y estable. Con excelente agarre, alta capacidad de carga y durabilidad. Puedes ajustar el anillo a la altura perfecta.
El anillo es adecuado para fisioterapia profesional o para el entrenamiento en casa. Con el anillo de madera estarás perfectamente equipado para rehabilitación, entrenamiento propioceptivo y para el entrenamiento de preparación para muchos tipos de deportes.
Fabricado con fibras de madera de alta calidad, es fisiológicamente inofensivo, fácil de lavar, inodoro y robusto. El anillo de gimnasia es resistente y duradero. La superficie antideslizante garantiza un entrenamiento seguro.

Características
Espesor: 28mm
Color: Madera.

Contenido del paquete:
2 anillo de gimnasia
2 cinturones de seguridad.`,
    image: "/images/anillas.webp",
    category: "equipamento",
    subcategory: "pesas",
    url: "https://mercadolibre.com/sec/1y5iZ8s",
    recommended: true,
    active: true
  },
  {id: 31,
    slug: "zapatilla-charged-assert-10-hombre-negro-under-armour",
    name: "Zapatilla Charged Assert 10 Hombre Negro Under Armour",
    description: " Estas son Zapatillas de Running que cualquiera puede usar para ir más rápido, con Charged Cushioning® para ayudar a proteger contra impactos, una parte superior transpirable y durabilidad por kilómetros.",
    descriptionLong: `Under Armour te hace mejor. En eso nos enfocamos. Inspiramos a todo apasionado del deporte con soluciones de alto rendimiento que no sabías que necesitabas, pero que no te puedes imaginar viviendo sin ellas. Todo lo que hacemos gira en torno al rendimiento humano, entregando lo necesario para empujar más allá de cualquier límite. Todos los productos que creamos se hacen para hacer mejor a todos los deportistas.

* Calzado publicado en talla chilena, por favor revisar la guía de tallas publicada en las fotos del producto para encontrar tu talla ideal en base a los CM de tu pie
* Zapatillas Run Chrgd Assrt 10 hombre Negro Under Armour
* Estas son Zapatillas de Running que cualquiera puede usar para ir más rápido, con Charged Cushioning® para ayudar a proteger contra impactos, una parte superior transpirable y durabilidad por kilómetros.
`,
    image: "/images/charger.webp",
    category: "accesorios",
    subcategory: "calzado",
    url: "https://mercadolibre.com/sec/2MFnrzP",
    recommended: true,
    active: true
  },
  {id: 32,
    slug: "short-secado-rapido-hombre-deportivo-upf50",
    name: "Short Secado Rápido Hombre Deportivo Upf50",
    description: "SHORT SECADO RAPIDO HOMBRE DEPORTIVO - UNISEX ENVIO GRATIS - POR LA COMPRA 3 UNIDADES",
    descriptionLong: `Características
.El producto es 95% poliester 5% spandex
.Ligero ideal para el verano
.Cordon ajustable
.No se plancha
.Modelo secado rapido`,
    image: "/images/short.webp",
    category: "accesorios",
    subcategory: "accesorios",
    url: "https://mercadolibre.com/sec/2HL2Hdi",
    recommended: true,
    active: true
  },
  {id: 33,
    slug: "set-mancuernas-barra-ajustables-20-kg-3-en-1-color-rosa-chicle",
    name: "Set Mancuernas Barra Ajustables 20 Kg 3 En 1 Color Rosa Chicle",
    description: "El set de mancuernas barra ajustables 20 kg 3 en 1 en color rosa chicle es una opción elegante y funcional para quienes buscan un equipo de entrenamiento completo y versátil.",
    descriptionLong: `El set de mancuernas barra ajustables 20 kg 3 en 1 en color rosa chicle es una opción elegante y funcional para quienes buscan un equipo de entrenamiento completo y versátil. Con un diseño moderno y atractivo, este set es perfecto tanto para principiantes como para atletas que desean realizar ejercicios de tonificación y fuerza en casa o en el gimnasio.

Este set incluye mancuernas que se pueden convertir en barra, permitiendo realizar una gran variedad de ejercicios, desde levantamientos de pesas tradicionales hasta entrenamientos más especializados. Con un peso total de 20 kg, se puede ajustar el peso de las mancuernas o barra según las necesidades del entrenamiento, ofreciendo la flexibilidad de modificar la carga a medida que progresas.

El color rosa chicle le da un toque único y distintivo, mientras que su estructura resistente garantiza durabilidad y seguridad en cada sesión de ejercicio. Las empuñaduras son ergonómicas y antideslizantes, proporcionando un agarre cómodo y seguro durante el uso. Los discos de peso son fáciles de montar y desmontar, lo que permite una rápida personalización del peso en función del tipo de entrenamiento.

Gracias a su diseño 3 en 1, que combina mancuernas y barra, este set es ideal para realizar una amplia variedad de ejercicios de fuerza y resistencia, como sentadillas, press de banca, curl de bíceps, press militar y muchos más. Es una excelente opción para quienes buscan maximizar sus entrenamientos con un solo equipo, sin ocupar mucho espacio y con un estilo único.`,
    image: "/images/pesasrosa.webp",
    category: "equipamento",
    subcategory: "pesas",
    url: "https://mercadolibre.com/sec/2me6RWj",
    recommended: true,
    active: true
  },
  {id: 34,
    slug: "toalla-baño-viaje-de-microfibra-de-secado-rápido-3-pcs",
    name: "Toalla Baño Viaje De Microfibra De Secado Rápido, 3 Pcs",
    description: "Toalla de viaje ultracompacta: Toalla de viaje multifuncional de 3 de tamaño: 38 x 38cm, 80 x 40cm, 152 x 76cm. La toalla ultrafina de secado rápido es perfecta para su bolsa de gimnasia, mochila o maleta..",
    descriptionLong: `Producto: Juego de toallas deportivas absorbentes de secado rápido

* La función principal:
-Toalla de viaje ultracompacta: Toalla de viaje multifuncional de 3 de tamaño: 38 x 38cm, 80 x 40cm, 152 x 76cm. La toalla ultrafina de secado rápido es perfecta para su bolsa de gimnasia, mochila o maleta.

-Toalla de terciopelo de microfibra antibacteriana de doble cara: la velocidad de secado de la microfibra es 10 veces mayor que la de las toallas de algodón ordinarias. La práctica bolsa de almacenamiento de malla mantiene sus toallas de playa limpias y fáciles de guardar mientras viaja.

-Botones independientes: se añaden botones independientes a las toallas de baño, que se pueden colgar en varios lugares, haciéndolo más diversificado y cómodo de usar.

-La bolsa de malla es de tamaño pequeño, fácil de almacenar y transportar, muy adecuada para viajes de negocios y viajes de negocios, ya que tiene fibras sintéticas, no retendrá el olor a algodón.
`,
    image: "/images/toallamicrofibra.webp",
    category: "accesorios",
    subcategory: "accesorios",
    url: "https://mercadolibre.com/sec/1MGpG4u",
    recommended: true,
    active: true
  },
  {id: 35,
    slug:"serum-antiarrugas-pure-vitamin-c12-30ml-la-roche-posay-todo-tipo-de-piel-noche",
    name: "Serum Antiarrugas Pure Vitamin C12 30ml La Roche-posay Todo Tipo De Piel Noche",
    description: "Serum Antiarrugas Pure Vitamin C12 30ml. Noche. Fórmula enriquecida con vitamina C pura al 12%",
    descriptionLong: `Serum Antiarrugas Pure Vitamin C12 30ml

La Roche-Posay

Todo Tipo De Piel

Noche

- Fórmula enriquecida con vitamina C pura al 12%
- Ayuda a reducir la apariencia de arrugas y líneas de expresión
- Textura ligera que se absorbe rápidamente
- Mejora la luminosidad y uniformidad del tono de la piel
- Contiene ingredientes hidratantes para mayor confort
- Ideal para uso nocturno

Ideal para revitalizar la piel y proporcionar un aspecto más joven y fresco.
`,
image: "/images/serumlarocheamarillo.webp",
    category: "belleza",
    subcategory: "belleza",
    url: "https://mercadolibre.com/sec/31zaQFQ",
    recommended: true,
    active: true
  },
  {id: 36,
    slug:"perfume-de-mujer-lattafa-yara-eau-de-parfum-100-ml",
    name: "Perfume de mujer Lattafa Yara eau de parfum 100 ml",
    description: "El perfume Yara de Lattafa presenta un aroma tropical y gourmand que lo distingue en el mundo de la perfumería.",
    descriptionLong: `El perfume Yara de Lattafa presenta un aroma tropical y gourmand que lo distingue en el mundo de la perfumería. Este perfume tiene una composición olfativa muy interesante que mezcla notas florales y frutales con un toque dulce y cremoso. Aquí están las notas detalladas:

• Notas de salida: Orquídea, heliotropo y mandarina.

• Notas de corazón: Acorde gourmand y frutas tropicales.

• Notas de fondo: Vainilla, almizcle y sándalo.

Detalles:
- Fragancia Femenina.
`,
image: "/images/perfumelattafa.webp",
    category: "belleza",
    subcategory: "perfumes",
    url: "https://mercadolibre.com/sec/24zptYm",
    recommended: true,
    active: true
  },
  {id: 37,
    slug:"pack-revitalift-serum-acido-hialuronico-serum-retinol",
    name: "Pack Revitalift Serum Ácido Hialurónico + Serum Retinol",
    description: `Para combatir los signos del envejecimiento descubre nuestro nuevo Serum L'Oreal Revitalift, SÉRUM RELLENADOR con 1.5% de Ácido Hialurónico Puro.`,
    descriptionLong: `El ÁCIDO HIALURÓNICO PURO es uno de los componentes naturales de la piel, pero con el tiempo va disminuyendo. La piel se deshidrata y las arrugas aparecen.


Para combatir estos signos del envejecimiento descubre nuestro nuevo Serum L'Oreal Revitalift, SÉRUM RELLENADOR con 1.5% de Ácido Hialurónico Puro. 0.5% de Ácido Hialurónico Hidrofílico Rehidrata intensamente la superficie de la piel al atraer y absorber 1000 veces su peso en agua.


1% de Ácido Hialurónico Fragmentado Penetra más profundo en la superficie de la piel para ayudar a rellenarla visiblemente.


RESULTADOS VISIBLES Inmediatamente

La piel se siente hidratada, más elástica y fresca.

En 1 semana*: La piel luce radiante, se siente como rellenada y firme.

En 2 semanas**: Las líneas de expresión se reducen (-19%).

Las arrugas se reducen (-14%). En 4 semanas**: Las líneas de expresión se reducen (-40%).

Las arrugas se reducen (-31%). Cada mañana y/o noche, aplica 2-3 gotas en el rostro y cuello, evitando el área de los ojos. Para mejores resultados, usa el resto de la gama.


Nuevo Revitalift Sérum Noche con Retinol L’Oreal París. Un nuevo avance con retinol puro que busca ofrecer la mejor eficacia y experiencia en un sérum. Consigue un efecto corrector de arrugas, incluso, profundas en las zonas clave del rostro.`,
    image: "/images/revitalif.webp",
    category: "belleza",
    subcategory: "belleza",
    benefits: [
    "El Serum Retinol Puro de L’Oréal Paris corrige arrugas, incluso las profundas.",
    "Posterior a su uso inmediatamente la piel está más hidratada, flexible y se ve más lisa.",
    "En 1 mes la textura de la piel se refina, se ve más joven y más luminosa.",
    "En 4 meses las arrugas se reducen un 36%*. Reducción visible de arrugas profundas.",
    ],
    url: "https://mercadolibre.com/sec/2HmsqHq",
    recommended: true,
    active: true
  },
  {id: 38,
    slug:"pack-cicatricure-pack-gold-lift-crema-dia-noche-serum",
    name: "Pack Cicatricure Pack Gold Lift Crema Dia+noche+serum",
    description: `Pack Cicatricure Gold Lift Crema Dia+Noche+Serum. La línea Antiedad Gold Lift combina Péptidos de Oro, Calcio y Silicio, impactando en la morfología de la piel, reduciendo la profundidad de las arrugas gravitacionales y redefiniendo el contorno facial.`,
    descriptionLong: `¿Qué contiene el pack?
- Cicatricure Gold Lift Serum facial 27 ml.
- Cicatricure Gold Lift Crema Día FPS 30 100 ml.
- Cicatricure Gold Lift Crema Noche 100 ml.

La línea Antiedad Gold Lift combina Péptidos de Oro, Calcio y Silicio, impactando en la morfología de la piel, reduciendo la profundidad de las arrugas gravitacionales y redefiniendo el contorno facial.

Favorece la recuperación de elasticidad y el proceso de auto reparación nocturna de la piel. Actúa en la noche creando una lámina que regula la pérdida de agua. Textura sedosa. Mejora la hidratación y la apariencia de la piel.

El serum facial de la línea Gold actúa sobre arrugas gravitacionales y líneas de expresión, su fórmula concentrada con Péptidos de Oro, Calcio y Silicio actúa con efecto lifting y se absorbe rápidamente, permitiendo una reparación más profunda de la piel en menos tiempo. Su efecto tensor mejora la elasticidad y humectación de la piel y con el uso continuado incrementa la firmeza, dando como resultado un rostro más definido. En solo 2 semanas esculpe y reafirma la piel y reduce la aparición de arrugas gravitacionales. En 6 semanas, la mayoría de las usuarias notaron su piel visiblemente más rejuvenecida. Aplícalo sobre rostro, cuello y escote perfectamente limpios. Dermatológicamente Testado.

Modo de uso: Diario
Para las cremas, aplicar diariamente, con movimientos ascendentes en rostro y cuello. La crema de noche se aplica de la misma forma antes de dormir.

Para el Serum, sobre rostro limpio y seco, aplicar algunas gotas del elixir y distribuir con pequeños golpecitos hasta que la piel lo absorba por completo. Se recomienda el uso diario.`,
    image: "/images/cicatricure.webp",
    category: "belleza",
    subcategory: "belleza",
    benefits: [
    "Reduce arrugas gravitacionales: Las cremas reducen el efecto de cara caída causado por los años y la gravedad, rellenando surcos y creando un efecto lifting en solo 2 semanas, utilizando la rutina completa de Cicatricure Gold Lift.",
    " Aumenta la elasticidad y la humectación: El Serum mejora al instante la elasticidad y humectación de la piel y con el tiempo incrementa la firmeza, dando como resultado un rostro más definido. Efecto tensor que redefine el contorno facial, reafirmando rostro, cuello, papada y escote. La crema de noche actúa mientras duermes.",
    "El serum actúa sobre arrugas gravitacionales y líneas de expresión. La mayoría de las usuarias sintieron su piel más firme.",
    "Actúa con efecto lifting y se absorbe rápidamente, permitiendo una reparación más profunda de la piel en menos tiempo.",
    ],
    url: "https://mercadolibre.com/sec/2FF2Rjp",
    recommended: true,
    active: true
  },
  {id: 39,
    slug:"perfume-lbel-bleu-intense-de-100-ml-edt-para-hombre-herbales-fresco-de-larga-duracion",
    name: "Perfume L'bel Bleu Intense De 100 ml EDT Para Hombre Herbales Fresco De Larga Duración",
    description: `Perfume Bleu Intense L'bel Para Hombre Notas Herbales 100ml. Fragancia masculina fresca y enérgica que combina notas herbales y marinas con la intensidad de la salvia.`,
    descriptionLong: `Perfume Bleu Intense L'bel Para Hombre Notas Herbales 100ml

Fragancia masculina fresca y enérgica que combina notas herbales y marinas con la intensidad de la salvia. Inspirada en la fuerza del océano, es ideal para hombres que viven sin límites y buscan un aroma revitalizante y duradero.

Características:
- Tipo de producto: perfume para hombre
- Familia olfativa: herbal aromática con notas marinas
- Concentración: Eau de Parfum
- Contenido: 100 ml
- Uso: versátil, para el día y cualquier ocasión

Notas olfativas:
- Salida: notas marinas, anís estrellado, cardamomo
- Cuerpo: baya de enebro, jengibre de China, salvia sclarea
- Fondo: haba tonka, Healingwood Captive, musgo de roble de Yugoslavia

Modo de uso:
- Aplica en cuello, muñecas y detrás de las orejas para maximizar la fijación y proyección.

Un aroma fresco y poderoso que transmite energía, vitalidad y confianza.`,
    image: "/images/bluehombre.webp",
    category: "belleza",
    subcategory: "perfumes",
    url: "https://mercadolibre.com/sec/1571XDM",
    recommended: true,
    active: true
  },
  {id: 40,
    slug:"protector-solar-corporal-para-niños-fps-50-eucerin-250-m",
    name: "Protector Solar Corporal Para Niños Fps 50+ | Eucerin 250 M",
    description: `Protector solar para niños especialmente formulado para calmar la piel sensible y protegerla de las quemaduras solares y el daño inducido por el sol a largo plazo.`,
    descriptionLong: `Protector solar para niños especialmente formulado para calmar la piel sensible y protegerla de las quemaduras solares y el daño inducido por el sol a largo plazo.

Características:
- Tipo de producto: protector solar
- Familia olfativa: sin notas específicas
- Concentración: FPS 50+
- Contenido: 250 ml
- Uso: diario, ideal para niños

Modo de uso:
- Aplica en todo el cuerpo, evitando los ojos.
- Reaplica cada 2 horas o después de nadar o sudar.`,
    image: "/images/protectoreucerin.webp",
    category: "belleza",
    subcategory: "belleza",
    url: "https://mercadolibre.com/sec/1Z3T4gL",
    recommended: true,
    active: true
  },
  {id: 41,
    slug:"gel-limpiador-espumoso-473-ml-tipo-de-piel-normal-a-grasa",
    name: "Gel Limpiador Espumoso 473 Ml Tipo de piel Normal a Grasa",
    description: `Gel Limpiador Espumoso CeraVe remueve y limpia gentilmente la suciedad y el exceso de grasa de la piel, ayudando a restaurar la barrera de la piel gracias a sus 3 ceramidas esenciales.`,
    descriptionLong: `Gel Limpiador Espumoso CeraVe remueve y limpia gentilmente la suciedad y el exceso de grasa de la piel, ayudando a restaurar la barrera de la piel gracias a sus 3 ceramidas esenciales.

Propiedades:
Desarrollada junto con dermatólogos, limpia profundamente sin resecar la piel.

Textura:
Gel espumoso, refrescante y de rápida absorción.

Modo de uso:
1. Presiona el dosificador.
2. Aplica el producto con masajes suaves en rostro, cuello y/o rostro.
3. Limpiar 1 o 2 veces al día según necesidad.
`,
    image: "/images/cerave.webp",
    category: "belleza",
    subcategory: "belleza",
    url: "https://mercadolibre.com/sec/2xRCWBG",
    benefits: [
    "Limpia profundamente.",
    "Refresca y limpia.",
    "Para piel Normal a Grasa.",
    "No comedogénico: No obstruye poros.",
    "Hipoalergénico.",
    "Apto para pieles sensibles.",
    ],
    recommended: true,
    active: true
  },
  {id: 42,
    slug:"serum-concentrado-mela-b3-la-roche-posay-antimanchas-30ml-con-melasyl-y-10-niacinamida-previene-la-reaparicion",
    name: "Sérum Concentrado Mela B3 La Roche-Posay Antimanchas 30ml con Melasyl y 10% Niacinamida Previene la Reaparición",
    description: `El Sérum Concentrado MELA B3 es un tratamiento específico formulado para combatir las manchas oscuras y prevenir su reaparición.`,
    descriptionLong: `La Roche Posay es una marca que ofrece una variedad de productos para el cuidado de la piel facial. Si buscas protegerla y mantenerla en forma, esta puede ser una buena opción.

El Sérum Concentrado MELA B3 es un tratamiento específico formulado para combatir las manchas oscuras y prevenir su reaparición. Potenciado con Melasyl™ y un 10% de Niacinamida, este sérum ayuda a reducir las manchas existentes y uniforma el tono de la piel. Es ideal para personas que desean una piel más clara y homogénea, reduciendo la aparición de nuevas manchas causadas por factores como el sol, el envejecimiento o cambios hormonales.`,
    image: "/images/serumlaroche.webp",
    category: "belleza",
    subcategory: "belleza",
    benefits: [
    "Reducción de Manchas: Disminuye la intensidad y el tamaño de las manchas oscuras en la piel.",
    "Prevención de Reaparición: Su fórmula avanzada evita que las manchas regresen, garantizando una piel más uniforme a largo plazo.",
    "Efecto Calmante e Iluminador: La combinación de Melasyl y Niacinamida ayuda a calmar la piel y mejorar su luminosidad.",
    "Apto para Todo Tipo de Piel: Su textura ligera y de rápida absorción lo hace adecuado para pieles normales, mixtas y sensibles.",
    ],
    url: "https://mercadolibre.com/sec/1nbJEVw",
    recommended: true,
    active: true
  },
  {
    id: 100,
    name: "Proteína Vegetal X",
    description: "Saciedad y digestión ligera",
    descriptionLong: ``,
    image: "/img/proteina.jpg",
    category: "suplementos",
    benefits: [
    "Reducción de Manchas: Disminuye la intensidad y el tamaño de las manchas oscuras en la piel.",
    "Prevención de Reaparición: Su fórmula avanzada evita que las manchas regresen, garantizando una piel más uniforme a largo plazo.",
    "Efecto Calmante e Iluminador: La combinación de Melasyl y Niacinamida ayuda a calmar la piel y mejorar su luminosidad.",
    "Apto para Todo Tipo de Piel: Su textura ligera y de rápida absorción lo hace adecuado para pieles normales, mixtas y sensibles.",
    ],
    url: "LINK_AFILIADO",
    recommended: true,
    active: true
  }
];
